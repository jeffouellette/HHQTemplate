/*
 $Id: runglauber.C 159 2018-04-30 07:45:40Z loizides $
 -------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
void runAndSmearNucleons(const Int_t n1, // first event label, e.g. 1000
                         const Int_t n2, // last event label, e.g. 1500; in this case 1500-1000=500 events will be generated
                         const char *sysA        = "Pb",
                         const char *sysB        = "Pb",
                         const Double_t signn    = 67.6, // using 42 for 200GeV, 72 for 8.16TeV
                         const Double_t sigwidth = -1, // using 0.5
                         const Double_t mind     = 0.4, // using 0.4
                         const Bool_t verbose    = 0, 
                         const char *fname       = 0, // name of output root file, e.g. "outFile_pPb.root"
                         const Double_t maxb     = -1); // force a maximum impact parameter if > 0
...
class TGlauNucleus : public TNamed
{
  ...
  public:
    ...
    TString    GetFName()         const {return fName;}
...
class TGlauberMC : public TNamed
{
  ...
  public:
    ...
    static const char  *Version()                    {return "v3.1";}
...
//  const Double_t e0_scal=(eoT4[temp])*(Ti[temp]*AT)*(Ti[temp]*AT)*(Ti[temp]*AT)*(Ti[temp]*AT); // e0 = eoT4 * T^4 * dx^4; units: [e0] = GeV, comes out as 0.00193359 typically
  ...
  Double_t e0;
  //e0 = 0.00170 * TMath::Power(140./(float)nbins, 4); // value for minbias pPb at 8TeV
  ...
  for (Int_t ievent=n1; ievent<n2; ++ievent) {
    ...
    e0 = e0_scal + ((ievent-n1)/100)*de0_scal; // find the scaling parameter e0 for this event
    ...
    /*** now store the initial condition in a root file ****/
...
void TGlauNucleus::Lookup(const char* name)
{
  ...
  else if (TString(name) == "Xesrw")   {fN = 129; fR = 5.32*1.019; fA = 0.57;   fW =  0;       fF = 12; fZ=54; r0=1.0096; r1=-0.000874123; r2=-0.000256708;}
...
TVector3 &TGlauNucleus::ThrowNucleons(Double_t xshift)
{
 ...
 cmscheck: /* start over here in case shift was too large */
  ...
  const Bool_t hulthen = (fName.EqualTo("dh") || fName.EqualTo("dhh"));
  const Bool_t helium3 = (fName.EqualTo("He3") || fName.EqualTo("H3"));
  ...
  fEv()
{
  ...
  TString name(Form("Glauber_%s_%s",fANucleus.GetFName().Data(),fBNucleus.GetFName().Data()));
  TString title(Form("Glauber %s+%s Version",fANucleus.GetFName().Data(),fBNucleus.GetFName().Data()));
  SetName(name);
  SetTitle(title);
}
...
Bool_t TGlauberMC::ReadNextEvent(Bool_t calc, const char *fname)
{
  ...
  if (fname) {
    ...
    if (calc)
      return 1;
    fNt = dynamic_cast<TNtuple*>(inf->Get(Form("nt_%s_%s",fANucleus.GetFName().Data(),fBNucleus.GetFName().Data())));
    if (!fNt) {
