...
void Hist2Txt(const int firstEvent = 500, // first event to process
              const int lastEvent  = 550) // last event to process
{
  ...
  for (int eventNum = firstEvent; eventNum < lastEvent; eventNum++) {
    ...
    for (int xbin = 0; xbin <= nbins; xbin++) { // loops exactly nbins+1 times (so NUMT in params should be EXACTLY nbins+1)
