...
void diffusion(
         ...
         const string    inFileName2 = "quarkdist.root", // input Ncoll distribution file
         const string    inNcollDistName = "inited_event0_translated", // input Ncoll distribution name
         const string    inHeavyQuarkPtSpectrumFileName = "heavy_quark_pt.root", // input c, b initial pT spectra file name
         const bool      plotThings = false, // whether to create canvases for final results. Non-essential for large batch jobs
         const bool      yesbeauty = false, // only beauty, otherwise only charm
	       const int       NQuarks = 5000000,
	       ...
	       ) 
{
  ...
  // moved to below next loop so that N_timesteps is accurate
